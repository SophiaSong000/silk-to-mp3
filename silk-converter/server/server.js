...
function convertSilkToPcmToMp3(inputFile, outputFile) {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      // 尝试多种可能的路径
      let silkDecoderPath = 'silk_v3_decoder';
      ...
      if (fs.existsSync(localSilkDecoder)) {
        silkDecoderPath = localSilkDecoder;
      }
      
      // 检查环境变量中的silk_v3_decoder
      try {
        const whichOutput = execSync('where silk_v3_decoder', { encoding: 'utf8' });
        if (whichOutput && whichOutput.trim()) {
          silkDecoderPath = whichOutput.trim().split('\n')[0];
        }
      } catch (e) {
      ...
      const silkProcess = exec(silkCommand, (silkError, silkStdout, silkStderr) => {
        ...
        // 尝试不同的采样率和通道数
        const ffmpegCommand = `ffmpeg -f s16le -ar 24000 -ac 1 -i "${pcmFile}" -acodec libmp3lame -q:a 2 "${outputFile}"`;
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      const silkProcess = exec(silkCommand, (silkError, silkStdout, silkStderr) => {
        ...
        const ffmpegCommand = `ffmpeg -i "${wavFile}" -acodec libmp3lame -q:a 2 "${outputFile}"`;
        console.log('执行ffmpeg命令:', ffmpegCommand);
...
function convertWithFfmpegDirect(inputFile, outputFile) {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      // 检查环境变量中的ffmpeg
      try {
        const whichOutput = execSync('where ffmpeg', { encoding: 'utf8' });
        if (whichOutput && whichOutput.trim()) {
          ffmpegPath = whichOutput.trim().split('\n')[0];
        }
      } catch (e) {
...
app.listen(PORT, () => {
  ...
  try {
    ...
    const silkVersion = execSync('silk_v3_decoder -h', { timeout: 5000 }).toString();
    console.log('silk_v3_decoder可用:', silkVersion.substring(0, 100) + '...');
  } catch (e) {
    console.error('警告: silk_v3_decoder可能不可用:', e.message);
    console.error('请确保silk_v3_decoder已安装并添加到系统PATH中');
  }
  
  try {
    console.log('检查ffmpeg是否可用...');
    const ffmpegVersion = execSync('ffmpeg -version', { timeout: 5000 }).toString();
    console.log('ffmpeg可用:', ffmpegVersion.substring(0, 100) + '...');
  } catch (e) {
    console.error('警告: ffmpeg可能不可用:', e.message);
    console.error('请确保ffmpeg已安装并添加到系统PATH中');
  }
