...
function convertSilkToPcmToMp3(inputFile, outputFile) {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      // 第一步：将silk转换为pcm
      const silkCommand = `silk_v3_decoder "${inputFile}" "${pcmFile}"`;
...
function convertSilkToWavToMp3(inputFile, outputFile) {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      // 第一步：将silk转换为wav
      const silkCommand = `silk_v3_decoder "${inputFile}" "${wavFile}" -d`;
...
function convertWithFfmpegDirect(inputFile, outputFile) {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      // 尝试直接使用ffmpeg转换
      const ffmpegCommand = `ffmpeg -i "${inputFile}" -acodec libmp3lame -q:a 2 "${outputFile}"`;
      console.log('执行ffmpeg命令:', ffmpegCommand);
